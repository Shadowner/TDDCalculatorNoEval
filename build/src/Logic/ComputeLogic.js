...
function matchingBetweenSeparator(separator) {
    if (typeof separator == "string")
        return new RegExp(`(\\d+|\\[index=\\d+\\])[${separator == '^' ? '\\^' : separator}](\\d+|\\[index=\\d+\\])`, "g");
    return new RegExp(`(${separator.start == '' ? '(\\()' : '(\\b' + separator.start + '\\b\\()'}(.*?)${separator.end == '' ? '' : ' (\\b' + separator.end + '\\b)'}(\\){1}))`, "g");
}
function traductToAlgoElements(value, isChild = false, childNumber = 0) {
    if (isChild)
        console.log(`${"".padStart(childNumber, "\t")}Child(${childNumber}) to process : ${value}`);
    else
        console.log(`to process : ${value}`);
        ...
        for (const logic of logics) {
            ...
            if (typeof logic.separator == "string") {
                ...
                potentialIndex.forEach(index => {
                    ...
                    if (isChild)
                        console.log(`${"".padStart(childNumber + 1, "\t")}newString(${childNumber}) : ${calculationString}`);
                    else
                        console.log(`newString : ${calculationString}`);
                });
            }
        }
    }
    ...
    for (const logic of calculationOrderArray) {
        ...
        if (isChild)
            console.log(`${"".padStart(childNumber + 1, "\t")}stringUpdate : ${calculationString}`);
        else
            console.log(`stringUpdate : ${calculationString}`);
    }
    if (algoElements.length == 0)
        throw new Error("Algo element vide");
    if (isChild)
        console.log(`${"".padStart(childNumber, "\t")}EndChild(${childNumber})`);
